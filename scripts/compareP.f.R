compareP <-function(name1, file1, cols1=NA, name2, file2, cols2=NA, plots="BETA EAF logP"){
	...
	if (!is.na(cols1)) {names(dat1) <- unlist(strsplit(cols1," "))}; dat1$logP <- -log10(dat1$P)
	if (!is.na(cols2)) {names(dat2) <- unlist(strsplit(cols2," "))}; dat2$logP <- -log10(dat2$P)
	...
	for (var in unlist(strsplit(plots," "))) {
		X <- dat[[paste0(var,'_1')]]
		Y <- dat[[paste0(var,'_2')]]
