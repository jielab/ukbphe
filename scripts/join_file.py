...
def build_hash(input):
    ...
    c = 0
    line_count = 0;
    with codecs.open(input[0],'r', encoding = 'utf-8', errors='ignore') as fp2:
        for line in fp2:
            line = line.strip()
            ...
            if c == 0:
                ...
                try:
                    assert c == len(line_list)
                except:
                    print('file %s has inconsistent field numbers in line %d' % (input[0], line_count))
                    exit()
                def test(list):
                    # testing blank line
                    for a in list:
...
with codecs.open(input_list[0][0],'r', encoding='utf-8', errors = 'ignore') as fp_in:
    sep = input_list[0][1]
    col = input_list[0][2]
    count = 0
    cnt = 0
